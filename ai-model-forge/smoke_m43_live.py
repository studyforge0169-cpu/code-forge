"""M43 live HTTP smoke against the production FORGE_ROOT
(/home/user/ai-model-forge-data). M43 is READ-ONLY: it adds one narrow
MODEL-SCOPED access path,
GET /models/{id}/gates/decisions/by-verdict/{verdict}, answering
"which immutable gate-decision records of this model recorded this
loss-only comparison verdict?" — the model's authoritative M6 listing
filtered by the persisted top-level verdict recorded in each
GateDecision (the Optional[ComparisonVerdict] enum improved/regressed/
unchanged, recorded verbatim by the M6 run and deliberately DISTINCT
from the M41 decision result; NEVER recalculated from losses, deltas,
tolerances, policies or comparison records; no gate is re-evaluated),
registered BEFORE the generic /gates/decisions/{decision_id} route and
alongside by-policy (M23)/by-comparison (M34)/by-decision (M41). The
field is OPTIONAL: threshold-only gates keep verdict null, there is
deliberately NO route representing None, and null-verdict decisions
belong to NO by-verdict group (they stay listed in the generic M6
history). Unsupported verdict values are rejected with 422 by the
schema enum BEFORE the handler runs (never a registry-style 404; true
even for an unknown model); an unknown model with a VALID verdict is
404; a valid verdict with zero matching decisions is 200 + [] (never
404). Each record appears EXACTLY ONCE and the groups partition the
NON-NULL part of the M6 listing. The smoke must DISCOVER the
authoritative verdict distribution from the live M6 listing and the
schema enum (not assume it from an old report), prove exact listing
parity for ALL THREE groups, the None exclusion, the natural
valid-empty cases, clean 404/422 separation, byte-identical repeats
(x3), unchanged M2-M42 surfaces + dashboard hash + registries +
OpenAPI 75, and ZERO production storage growth.

Production facts (re-derived live at LIVE A/B; the authoritative
sources are the M6 listing and the ComparisonVerdict schema enum):
model 4a0a871886ef owns 11 gate decisions in ASCENDING (created_at,
decision_id) order with persisted verdict distribution improved -> 4,
regressed -> 3, unchanged -> 2 (all three groups non-empty) plus 2
threshold-only null-verdict decisions outside every group; model
...
SHA256 inventory saved to /tmp/m43-smoke-baseline-inventory.json +
model/checkpoint registries + M4/M5/M6/M11/M15/M16/M18-M42 pre-state
+ dashboard + OpenAPI 75 pre-state + the known pair returns 200.

LIVE B (authoritative distribution discovery): from the live M6
listing and the OpenAPI ComparisonVerdict schema enum, discover the
per-verdict record distribution INCLUDING the null-verdict count;
print them; cross-check against the audited facts.

LIVE C (A — known verdicts): improved, regressed AND unchanged -> 200
with the EXACT discovered records (4/3/2); parity with the M6 listing
filtered locally; verbatim detail-getter payloads; (created_at,
decision_id) ASC.

LIVE D (B — repeatability): three GETs raw-byte-identical.

LIVE E (C — valid empty verdicts): ALL enum verdicts -> 200 + []
under b5bc905326b6 (it has no gate decisions at all).

LIVE F (D/E — unknown ids / unsupported values): unknown model +
valid verdict -> 404; unsupported verdict value -> 422 on the known
model (incl. case variant + numeric + "worse"); the None contract —
"none"/"null" are NOT enum values -> 422 (no route represents None);
unsupported verdict + UNKNOWN model -> 422 (schema validation fires
pre-handler).

LIVE G (F — partition): the groups over all enum verdicts partition
the NON-NULL part of the M6 listing exactly (4 + 3 + 2 == 9; total
stays 11; every record EXACTLY ONCE; no record in two groups; the 2
null-verdict threshold-only decisions in NO group but listed).

LIVE H (G — ordering): every group's order equals the authoritative
M6 listing order restricted to that verdict.

LIVE I (H — M23/M34/M41 regressions): gate decisions 1 by-policy
(m9-live-policy) + 4 by-comparison (fc379bfcb50f) + by-decision
passed->7 / failed->4 + the M6 listing unchanged, all with listing
parity.

LIVE J (I — M22-M42 regressions): evaluations 3/3/3 by-checkpoint +
...
stopped->1 + 2 by-recipe (m12-live-suite).
...
LIVE M (M — OpenAPI + storage zero drift): OpenAPI exactly 75 paths,
new path once (after the by-decision gate route, before the generic
gate-decision route; the M42 workflows-by-status and M41
by-decision routes still present exactly once); every pre-existing
file byte-identical, ZERO new files, zero .tmp, totals unchanged
96/4,002,745/0.

Exit code 0 = all checks passed. Start the server first:
    FORGE_ROOT=/home/user/ai-model-forge-data python -m uvicorn \\
        app.api:app --host 127.0.0.1 --port 8764 --log-level warning
"""
...
BASE = "http://127.0.0.1:8764/api/v1"
SITE = "http://127.0.0.1:8764"
MODEL = "4a0a871886ef"
...
NULL_VERDICTS = 2                     # threshold-only decisions
EXPECTED_VERDICTS = ("improved", "regressed", "unchanged")
NEW_PATH = ("/api/v1/models/{model_id}/gates/decisions/by-verdict/"
            "{verdict}")
...
INVENTORY = Path("/tmp/m43-smoke-baseline-inventory.json")
...
def byverdict(verdict: str, model: str = MODEL) -> str:
    return (f"{BASE}/models/{model}/gates/decisions/by-verdict/"
            f"{verdict}")
...
def main() -> int:
    ...
    check("A4 M4/M5/M6/M11/M15/M16/M18-M42 pre-state intact (11 gate "
          "decisions, other model 0, 13 workflow runs, 16 evaluations, "
          "8 comparisons, 4 samples, 7 recipes, 2 global m12-live-suite "
          "runs, 2 by-recipe runs, 4 by-comparison decisions, 1 "
          "by-policy decision, 7 by-decision passed, 9 by-status "
          "completed workflows, 2 sample-quality, 10 by-suite, summary "
          "10, 10 by-checkpoint suite runs, 16/8/4/2 by-tokenizer "
          ...
          and gdecisions_o == [] and code_wf == 200
    ...
    check("A5 M17 dashboard hash equals the full known value",
          ...
          d0["result_hash"][:16])
    check("A6 OpenAPI pre-state: 75 paths, the new by-verdict path "
          "registered exactly once with only GET",
          code_sp == 200 and len(paths) == 75
          and list(paths).count(NEW_PATH) == 1 and set(new_ops) == {"get"}
          and new_ops["get"]["tags"] == ["gates"])
    code_a, raw_a = call("GET", byverdict("improved"))
    check("A7 the known pair (4a0a871886ef + improved) -> 200",
          code_a == 200, f"{code_a}")

    print("== LIVE B: authoritative distribution discovery ==")
    schema = spec["components"]["schemas"]["ComparisonVerdict"]
    ...
    for d in gdecisions:
        if d["verdict"] is None:
            ...
            by_v.setdefault(d["verdict"], []).append(d["decision_id"])
    dist_str = ", ".join(f"{k} -> {len(v)}"
                         for k, v in sorted(by_v.items()))
    print(f"    discovered: {len(gdecisions)} gate decisions of {MODEL} "
          f"per verdict: {dist_str}, null -> {n_none}")
    print(f"    discovered: ComparisonVerdict enum values: {list(enum)}")
    check("B1 M6 listing pre-state: 11 gate decisions in ASCENDING "
          "(created_at, decision_id) order; verdict distribution "
          "improved->4, regressed->3, unchanged->2 + null->2; other "
          "model has none",
          code_gd == 200 and len(gdecisions) == 11
          and [(d["created_at"], d["decision_id"]) for d in gdecisions]
          == sorted((d["created_at"], d["decision_id"])
                    for d in gdecisions)
          and {k: len(by_v.get(k, [])) for k in enum}
          == dict(M43_DIST)
          and n_none == NULL_VERDICTS
          and all(d["verdict"] in enum for d in gdecisions
                  if d["verdict"] is not None)
          and code_gdo == 200 and gdecisions_o == [])
    check("B2 the schema enum holds exactly improved/regressed/unchanged",
          set(enum) == set(EXPECTED_VERDICTS) and len(enum) == 3,
          str(enum))

    print("== LIVE C: (A) known verdicts — exact responses ==")
    groups: dict[str, list] = {}
    ok_groups = True
    for v in EXPECTED_VERDICTS:
        code_g, raw_g = call("GET", byverdict(v))
        groups[v] = json.loads(raw_g)
        ok_groups = ok_groups and code_g == 200 and groups[v] == \
            [d for d in gdecisions if d["verdict"] == v]
    check("C1/A1 ALL THREE verdict responses == M6 listing filtered "
          "locally by the persisted verdict (no missing / extra / "
          "duplicate; every record EXACTLY ONCE; the persisted verdict "
          "travels VERBATIM — never recalculated from losses or "
          "comparisons)",
          ok_groups
          and all(len(groups[v]) == M43_DIST[v]
                  and all(d["model_id"] == MODEL
                          and d["verdict"] == v
                          for d in groups[v])
                  for v in EXPECTED_VERDICTS),
          "/".join(str(len(groups[v])) for v in EXPECTED_VERDICTS))
    ok_detail = True
    for v in EXPECTED_VERDICTS:
        for d in groups[v]:
    ...
    if ok_detail:
        check("C2/A1 verbatim payload parity with the M6 detail getter "
              "for all 9 non-null records (decision/evidence chain/"
              "rollback suggestion included)", True)

    print("== LIVE D: (B) repeatability ==")
    _, r1 = call("GET", byverdict("improved"))
    _, r2 = call("GET", byverdict("improved"))
    _, r3 = call("GET", byverdict("improved"))
    check("D1/B1 three GETs raw-byte-identical", r1 == r2 == r3 == raw_a)

    print("== LIVE E: (C) valid empty verdicts ==")
    ok_other = True
    for v in enum:
        c_o, body_o = call_json("GET", byverdict(v, OTHER_MODEL))
        ok_other = ok_other and c_o == 200 and body_o == []
    check("E1/C1 ALL enum verdicts under b5bc905326b6 -> 200 + [] "
          "(its M6 listing is empty; the verdict value is "
    ...
    c1, _ = call_json("GET", byverdict("improved", "no-such-model-43"))
    check("F1/D1 unknown model + VALID verdict -> 404",
          c1 == 404, f"{c1}")
    c2, b2 = call_json("GET", byverdict("IMPROVED"))
    c3, b3 = call_json("GET", byverdict("improv%20ed"))
    c4, b4 = call_json("GET", byverdict("1"))
    c5, b5 = call_json("GET", byverdict("worse"))
    check("F2/E1 unsupported verdict values -> 422 on the known model "
          "(schema enum rejection — case variant, interior-space, "
          "numeric, non-enum word — never a 404, never [])",
          c2 == c3 == c4 == c5 == 422
          and all("enum" in json.dumps(b)
                  for b in (b2, b3, b4, b5) if b),
          f"{c2}/{c3}/{c4}/{c5}")
    c6, _ = call_json("GET", byverdict("none"))
    c7, _ = call_json("GET", byverdict("null"))
    check("F3/E1 the None contract: 'none'/'null' are NOT enum values "
          "-> 422 (there is deliberately NO route representing the "
          "null verdict; threshold-only decisions are reachable only "
          "through the generic M6 listing)",
          c6 == c7 == 422, f"{c6}/{c7}")
    c8, _ = call_json("GET", byverdict("worse", "no-such-model-43"))
    check("F4/E1 unsupported verdict + UNKNOWN model -> 422 "
    ...
    ok_groups2 = True
    for v in enum:
        _, body = call_json("GET", byverdict(v))
        ok_groups2 = ok_groups2 and all(d["verdict"] == v
                                        for d in body)
        seen |= {d["decision_id"] for d in body}
    non_null = {d["decision_id"] for d in gdecisions
                if d["verdict"] is not None}
    none_ids = {d["decision_id"] for d in gdecisions
                if d["verdict"] is None}
    check("G1/F1 the groups over all enum verdicts partition the "
          "NON-NULL part of the M6 listing exactly (4 + 3 + 2 == 9; "
          "total stays 11; every record EXACTLY ONCE; no record in two "
          "groups; the 2 null-verdict threshold-only decisions in NO "
          "group but listed)",
          ok_groups2 and seen == non_null and len(seen) == 9
          and len(none_ids) == 2
          and none_ids.isdisjoint(seen)
          and none_ids | non_null
          == {d["decision_id"] for d in gdecisions})

    print("== LIVE H: (G) ordering ==")
    ok_order = True
    for v in enum:
        _, body = call_json("GET", byverdict(v))
        expected = [d for d in gdecisions if d["verdict"] == v]
    ...
    print("== LIVE I: (H) M23/M34/M41 + M6 regressions ==")
    ...
    _, gd41p2 = call_json("GET", f"{BASE}/models/{MODEL}/gates/"
                                 "decisions/by-decision/passed")
    _, gd41f2 = call_json("GET", f"{BASE}/models/{MODEL}/gates/"
                                 "decisions/by-decision/failed")
    _, gdecisions2 = call_json("GET", f"{BASE}/models/{MODEL}/"
                                       "gates/decisions")
    check("I1/H1 M23/M34/M41 gate histories + the M6 listing unchanged "
          "(4 by-comparison with listing parity, 1 by-policy with "
          "listing parity, by-decision passed->7 / failed->4 with "
          ...
          and gd41f2 == [d for d in gdecisions
                         if d["decision"] == "failed"]
          and len(gd41f2) == M41_DIST["failed"]
          and gdecisions2 == gdecisions)

    print("== LIVE J: (I) M22-M42 regressions ==")
    ...
    check("M1 OpenAPI exactly 75 paths, the new path exactly once "
          "(after the by-decision gate route, before the generic "
          "gate-decision route; the M42 workflows-by-status and M41 "
          "by-decision routes still present exactly once)",
          code_sp2 == 200 and len(spec2["paths"]) == 75
          and list(spec2["paths"]).count(NEW_PATH) == 1
          and set(spec2["paths"][NEW_PATH]) == {"get"}
          and list(spec2["paths"]).index(M41_PATH)
    ...
    _, grouped2 = call_json("GET", byverdict("improved"))
    check("M2 by-verdict still deterministic at the end",
          grouped2 == groups["improved"])
    ...
    print()
    if FAILURES:
        print(f"M43 live smoke FAILED: {len(FAILURES)} check(s) "
              "failed:")
        for f in FAILURES:
            print("  -", f)
        return 1
    print("M43 live smoke OK: narrow read-only MODEL-SCOPED "
          "by-verdict grouping of the M6 gate-decision history "
          "(distribution DISCOVERED live: improved->4, regressed->3, "
          "unchanged->2 among 11 gate decisions plus 2 threshold-only "
          "null-verdict decisions — exact listing parity, persisted "
          "verdict identity VERBATIM (never recalculated from losses "
          "or comparisons, never re-evaluated), authoritative order, "
          "every record exactly once, full partition of the non-null "
          "records with None NEVER matching —, schema-enum 422 for "
          "unsupported verdict values even pre-handler and even for "
          "an unknown model, NO route representing None ('none'/"
          "'null' -> 422), valid empty 200 + [] for every verdict "
          "under b5bc905326b6, clean 404 for unknown model with a "
          "valid verdict, deterministic byte-identical x3 repeats, "
          "M12/M16/M22-M42 surfaces + dashboard hash + registries + "
          "OpenAPI 75 unchanged, ZERO production storage growth) "
