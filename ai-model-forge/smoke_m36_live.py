"""M36 live HTTP smoke against the production FORGE_ROOT
(/home/user/ai-model-forge-data). M36 is READ-ONLY: it adds one narrow
MODEL-SCOPED access path, GET /models/{id}/evaluations/by-split/{split},
answering "which immutable M4 evaluation records of this model were run
against this data split?" — the model's authoritative M4 listing filtered
by the persisted top-level split recorded in each EvaluationRecord
(matched VERBATIM against the EvaluationSplit schema enum — never
inferred from filenames, directories, timestamps, dataset names or ids),
registered BEFORE the generic /evaluations/{eval_id} route and alongside
by-checkpoint/by-dataset/by-tokenizer. Unsupported split values are
...
with a VALID split is 404; a valid split with zero matching records is
200 + [] (never 404). Each record appears EXACTLY ONCE and the groups
partition the full M4 listing. The smoke must DISCOVER the authoritative
split distribution from the live M4 listing and the schema enum (not
...
(x3), unchanged M2-M35 surfaces + dashboard hash + registries +
OpenAPI 68, and ZERO production storage growth.

Production facts (re-derived live at LIVE A/B; the authoritative
sources are the M4 listing and the EvaluationSplit schema enum): model
4a0a871886ef owns 16 evaluations in ASCENDING (created_at, eval_id)
order with persisted split grouping validation -> 14, train -> 2,
test -> 0 (test is the natural valid-enum empty case); model
b5bc905326b6 has NO evaluations; M17 dashboard result_hash
...
SHA256 inventory saved to /tmp/m36-smoke-baseline-inventory.json +
model/checkpoint registries + M4/M5/M6/M11/M16/M18-M35 pre-state +
dashboard + OpenAPI 68 pre-state + the known pair returns 200.
...
per-split record distribution and the enum values; print them;
cross-check against the audited facts.

LIVE C (A — known split): validation -> 200 with the EXACT discovered
records; parity with the M4 listing filtered locally; verbatim
detail-getter payloads; (created_at, eval_id) ASC.

LIVE D (B — repeatability): three GETs raw-byte-identical.

LIVE E (C — valid empty splits): test -> 200 + [] on the known model;
ALL enum splits -> 200 + [] under b5bc905326b6 (it has no evaluations
at all).
...
full M4 listing exactly (14 + 2 + 0 == 16; every record EXACTLY ONCE;
no record in two groups; every record's persisted split verbatim).

LIVE H (G — ordering): every group's order equals the authoritative
M4 listing order restricted to that split.

LIVE I (H — M24/M28/M30 regressions): evaluations 3/3/3
by-checkpoint + 16 by-dataset + 16 by-tokenizer + parity with the
filtered M4 listing.

LIVE J (I — M22-M34 regressions): suite runs 10 by-suite + 10
...
LIVE M (M — OpenAPI + storage zero drift): OpenAPI exactly 68 paths,
new path once (after the by-tokenizer route, before the generic
evaluation route); every pre-existing file byte-identical, ZERO new
...
Exit code 0 = all checks passed. Start the server first:
    FORGE_ROOT=/home/user/ai-model-forge-data python -m uvicorn \
        app.api:app --host 127.0.0.1 --port 8757 --log-level warning
"""
...
BASE = "http://127.0.0.1:8757/api/v1"
SITE = "http://127.0.0.1:8757"
MODEL = "4a0a871886ef"
...
EXPECTED_DIST = {"validation": 14, "train": 2, "test": 0}
NEW_PATH = "/api/v1/models/{model_id}/evaluations/by-split/{split}"
GENERIC_PATH = "/api/v1/models/{model_id}/evaluations/{eval_id}"
TOK_PATH = ("/api/v1/models/{model_id}/evaluations/by-tokenizer/"
            "{tokenizer_id}")
RECIPE_PATH = ("/api/v1/models/{model_id}/workflows/by-recipe/"
               "{recipe_id}")
ROOT = Path("/home/user/ai-model-forge-data")
INVENTORY = Path("/tmp/m36-smoke-baseline-inventory.json")
...
def bysplit(split: str, model: str = MODEL) -> str:
    return f"{BASE}/models/{model}/evaluations/by-split/{split}"
...
def main() -> int:
    ...
    check("A4 M4/M5/M6/M11/M16/M18-M35 pre-state intact (16 "
    ...
    check("A5 M17 dashboard hash equals the full known value",
          ...
          d0["result_hash"][:16])
    check("A6 OpenAPI pre-state: 68 paths, the new by-split path "
          "registered exactly once with only GET",
          code_sp == 200 and len(paths) == 68
    ...
    for e in evals:
        by_split.setdefault(e["split"], []).append(e["eval_id"])
    ...
    check("B1 M4 listing pre-state: 16 evaluations in ASCENDING "
          "(created_at, eval_id) order; distribution validation->14, "
          "train->2, test->0; other model has none",
          ...
          == dict(EXPECTED_DIST)
          and all(e["split"] in enum for e in evals)
    ...
    filtered = [e for e in evals if e["split"] == "validation"]
    ids = [e["eval_id"] for e in grouped]
    check("C1/A1 response == M4 listing filtered locally by the "
          "persisted split identity (no missing / extra / duplicate; "
          "every record EXACTLY ONCE; each record's persisted split "
          "verbatim)",
          code == 200 and grouped == filtered
          and len(ids) == len(set(ids)) == len(filtered) == 14
          and all(e["model_id"] == MODEL and e["split"] == "validation"
                  for e in grouped),
          f"{code}/{len(grouped)}")
    ok_detail = True
    for e in grouped:
    ...
    if ok_detail:
        check("C2/A1 verbatim payload parity with the M4 detail getter "
              "for all 14 records (metrics + split included)", True)
    ...
    c0, body0 = call_json("GET", bysplit("test"))
    check("E1/C1 test (valid enum value with zero matching records) -> "
          "200 + [] on the known model (NEVER 404)",
          c0 == 200 and body0 == [], f"{c0}")
    ok_other = True
    for sp in enum:
    ...
    check("E2/C1 ALL enum splits under b5bc905326b6 -> 200 + [] (its "
          "M4 listing is empty; the split value is schema-validated "
          "even when the model has no history)",
          ok_other and evals_o == [])

    print("== LIVE F: (D/E) unknown ids / unsupported values ==")
    c1, _ = call_json("GET", bysplit("validation", "no-such-model-36"))
    ...
    check("F2/E1 unsupported split values -> 422 on the known model "
          ...
          f"{c2}/{c3}/{c4}")
    c5, _ = call_json("GET", bysplit("banana-split", "no-such-model-36"))
    ...
    for sp in enum:
        ...
        ok_groups = ok_groups and all(e["split"] == sp for e in body)
        seen |= {e["eval_id"] for e in body}
    all_ids = {e["eval_id"] for e in evals}
    check("G1/F1 the groups over all enum splits partition the FULL M4 "
          "listing exactly (14 + 2 + 0 == 16; every record EXACTLY "
    ...
    for sp in enum:
        _, body = call_json("GET", bysplit(sp))
        expected = [e for e in evals if e["split"] == sp]
    ...
    print("== LIVE I: (H) M24/M28/M30 regressions ==")
    ok_ck = True
    ...
    _, evt2 = call_json("GET", f"{BASE}/models/{MODEL}/evaluations/"
                               f"by-tokenizer/{TOKENIZER}")
    check("I1/H1 M24/M28/M30 evaluation histories unchanged (3/3/3 "
          "by-checkpoint, 16 by-dataset, 16 by-tokenizer, parity with "
    ...
    print("== LIVE J: (I) M22-M34 regressions ==")
    ...
    code_c5, comps2 = call_json("GET", f"{BASE}/models/{MODEL}/comparisons")
    _, cmpd2 = call_json("GET", f"{BASE}/models/{MODEL}/comparisons/"
                                f"by-dataset/{DATASET}")
    _, cmpt2 = call_json("GET", f"{BASE}/models/{MODEL}/comparisons/"
                                f"by-tokenizer/{TOKENIZER}")
    check("J2/I1 M26/M29/M31 comparison histories unchanged (025e->6 / "
          "30a8->5 / 0511->1, 8 by-dataset, 8 by-tokenizer, parity with "
          "the filtered M5 listing)",
          ok_cmp and code_c5 == 200 and len(comps2) == 8
          and len(cmpd2) == M29_COUNT
          and cmpd2 == [x for x in comps2 if x["dataset_id"] == DATASET]
    ...
    check("M1 OpenAPI exactly 68 paths, the new path exactly once "
          "(after the by-tokenizer route, before the generic "
          "evaluation route; the M35 by-recipe route still present "
          "exactly once)",
          code_sp2 == 200 and len(spec2["paths"]) == 68
          ...
          and list(spec2["paths"]).count(RECIPE_PATH) == 1)
    ...
    print()
    if FAILURES:
        print(f"M36 live smoke FAILED: {len(FAILURES)} check(s) failed:")
        for f in FAILURES:
            print("  -", f)
        return 1
    print("M36 live smoke OK: narrow read-only MODEL-SCOPED by-split "
          "grouping of the M4 evaluation history (distribution "
          "DISCOVERED live: validation->14, train->2, test->0 among 16 "
          "evaluations — exact listing parity, persisted split "
          "identity VERBATIM, authoritative order, every record "
          "exactly once, full partition —, schema-enum 422 for "
          "unsupported split values even pre-handler and even for an "
          "unknown model, valid empty 200 + [] for test and for all "
          "splits under b5bc905326b6, clean 404 for unknown model with "
          "a valid split, deterministic byte-identical x3 repeats, "
          "M12/M16/M22-M35 surfaces + dashboard hash + registries + "
          "OpenAPI 68 unchanged, ZERO production storage growth) "
