"""M48 live HTTP smoke against the production FORGE_ROOT
(/home/user/ai-model-forge-data). M48 is READ-ONLY: it adds one narrow
MODEL-SCOPED access path,
GET /models/{id}/evaluations/by-seed/{seed},
answering "which immutable evaluations of this model ran with this
effective seed?" — the model's authoritative M4 listing
(deterministic (created_at, eval_id) ASCENDING order) filtered
VERBATIM by the REQUIRED integer EvaluationRecord.seed persisted on
every record at run time (the effective seed used, default derived
from the config; matched VERBATIM — NEVER recalculated, NEVER
normalized, NEVER derived from the embedded config dict, request
parameters, dataset identity, splits, tokenizers, state kinds,
losses, ids, timestamps or any other field; the seed is bookkeeping
identity, not a quality metric — no seed is better than another),
registered BEFORE the generic /evaluations/{eval_id} route and
alongside by-checkpoint (M24)/by-dataset (M28)/by-tokenizer
(M30)/by-split (M36)/by-state-kind (M38)/by-truncated (M47). The
seed is an OPEN integer value axis — no registry, no enum, no
artificial range constraint: any integer is type-valid (an unmatched
seed on a valid model returns 200 [] — a natural valid-empty), a
non-integer spelling is rejected 422 at the API boundary
(schema-level validation, pre-handler — integers are never silently
reinterpreted, and the validation fires BEFORE the handler even for
an unknown model), while an unknown model with a VALID integer is
404. The smoke must DISCOVER the authoritative seed distribution
from the live M4 listing (not assume it from an old report), prove
exact listing parity for ALL discovered groups, the natural
valid-empty, clean 404/422 separation, byte-identical repeats (x3),
unchanged M3-M47 surfaces + dashboard hash + registries + OpenAPI
80, and ZERO production storage growth.
...
eval_id) order with persisted seed distribution 11 -> 4, 1 -> 3,
7 -> 2, 3 -> 2, 7002 -> 2, 7101 -> 2, 12 -> 1 (SEVEN populated
groups; 4 + 3 + 2 + 2 + 2 + 2 + 1 == 16 — a TRUE disjoint
partition; the int is REQUIRED on every record, no None case);
model b5bc905326b6 has NO evaluations; M17 dashboard result_hash
...
SHA256 inventory saved to /tmp/m48-smoke-baseline-inventory.json +
model/checkpoint/tokenizer/policy/probe-suite/recipe registries +
M4-M47 pre-state (16 evaluations, 8 comparisons, 11 gate decisions,
13 workflows, 10 suite-runs, 4 samples, 3 checkpoints with run
distribution 2/1, truncation false->15 / true->1) + dashboard +
OpenAPI 80 pre-state + the known pair returns 200.

LIVE B (authoritative distribution discovery): from the live M4
listing, discover the persisted seed distribution; print it;
cross-check against the audited facts (seven groups summing to 16).

LIVE C (A — known seeds): ALL discovered seeds -> 200 with the
EXACT discovered records; parity with the M4 listing filtered
locally by the persisted record value; verbatim detail-getter
payloads for all 16 records; (created_at, eval_id) ASC preserved
per group.
...
has no evaluations at all); the unmatched seed on the KNOWN model
...
LIVE G (E — true partition): the per-seed groups form a TRUE
disjoint partition of the FULL M4 listing (pairwise disjoint;
union == all 16; every record EXACTLY ONCE; no None case — the int
is required on every record).

LIVE H (F — ordering): every group's order equals the authoritative
M4 (created_at, eval_id) ASCENDING order restricted to that seed.

LIVE I (G — M4/M24/M28/M30/M36/M38/M47 regressions): the evaluation
listing identical to LIVE A; by-checkpoint 3/3/3; by-dataset 16;
by-tokenizer 16; by-split validation->14 / train->2 / test->0;
by-state-kind checkpoint->9 / current->7; by-truncated false->15 /
true->1 — all with listing parity.
...
LIVE M (L — OpenAPI + storage zero drift): OpenAPI exactly 80 paths,
new path once (after by-truncated, before the generic evaluation
detail route; integer parameter schema; EvaluationRecord items; the
M47/M46/M45/M44/M43/M42/M41 routes still present exactly once);
...
Exit code 0 = all checks passed. Start the server first:
    FORGE_ROOT=/home/user/ai-model-forge-data python -m uvicorn \
        app.api:app --host 127.0.0.1 --port 8769 --log-level warning
"""
...
BASE = "http://127.0.0.1:8769/api/v1"
SITE = "http://127.0.0.1:8769"
MODEL = "4a0a871886ef"
...
NEW_PATH = "/api/v1/models/{model_id}/evaluations/by-seed/{seed}"
...
INVENTORY = Path("/tmp/m48-smoke-baseline-inventory.json")
...
def byseed(seed, model: str = MODEL) -> str:
    return f"{BASE}/models/{model}/evaluations/by-seed/{seed}"
...
def main() -> int:
    ...
    check("A4 M4-M47 pre-state intact (16 evaluations, other model 0, "
          "truncation false->15 / true->1, 3 checkpoints, 8 "
          "comparisons, 11 gate decisions, 13 workflows, 4 samples, "
          "10 suite runs, 7 recipes, OpenAPI 80 with the by-seed "
          "path registered exactly once, GET-only, tag evaluation)",
          ...
          and sum(1 for e in evals if e["truncated"]) == 1
          ...
          == 7
          and code_sp == 200 and len(paths) == 80
    ...
    check("A5 M17 dashboard hash equals the full known value",
          ...
          d0["result_hash"][:16])
    code_a, raw_a = call("GET", byseed(11))
    check("A6 the known pair (4a0a871886ef + seed 11) -> 200",
          code_a == 200, f"{code_a}")

    print("== LIVE B: authoritative distribution discovery ==")
    dist: dict = {}
    for e in evals:
        dist[e["seed"]] = dist.get(e["seed"], 0) + 1
    ...
    check("B1 M4 listing pre-state: 16 evaluations in ASCENDING "
          "(created_at, eval_id) order; persisted seed distribution "
          "11->4 / 1->3 / 7->2 / 3->2 / 7002->2 / 7101->2 / 12->1 "
          "(SEVEN populated groups summing to 16 — a TRUE disjoint "
          "partition; the int is REQUIRED, no None case); other model "
          "has none",
          code_ev == 200 and len(evals) == 16
          and [(e["created_at"], e["eval_id"]) for e in evals]
          == sorted((e["created_at"], e["eval_id"]) for e in evals)
          and dist == dict(M48_DIST)
          and all(isinstance(e["seed"], int) for e in evals)
          and sum(dist.values()) == 16
    ...
    for seed in dist:
        ...
        ok_groups = ok_groups and code_g == 200 and groups[seed] == \
            [e for e in evals if e["seed"] == seed]
    check("C1/A1 ALL SEVEN seed responses == M4 listing filtered "
          ...
          "never derived from the config dict or any other field)",
          ok_groups
          and all(len(groups[s]) == dist[s]
                  and all(e["model_id"] == MODEL and e["seed"] == s
                          for e in groups[s])
                  for s in dist),
    ...
    _, r1 = call("GET", byseed(11))
    _, r2 = call("GET", byseed(11))
    _, r3 = call("GET", byseed(11))
    check("D1/B1 three GETs raw-byte-identical", r1 == r2 == r3 == raw_a)

    print("== LIVE E: (C) valid empty seeds ==")
    c_pop_o, body_pop_o = call_json("GET", byseed(11, OTHER_MODEL))
    ...
    check("E1/C1 a populated production seed AND an unmatched valid "
          "integer under b5bc905326b6 -> 200 + [] (its M4 listing is "
          ...
          and c_unm == 200 and body_unm == [] and evals_o == [])

    print("== LIVE F: (D) unknown ids / non-integer spellings ==")
    c1, _ = call_json("GET", byseed(11, "no-such-model-48"))
    ...
    c5, _ = call_json("GET", byseed("abc", "no-such-model-48"))
    ...
    for seed in dist:
        ...
        ok_groups2 = ok_groups2 and len(ids) == len(set(ids)) \
            and all(e["seed"] == seed for e in body)
        ids_by[seed] = set(ids)
    ...
    check("G1/E1 the per-seed groups form a TRUE disjoint partition of "
          "the FULL M4 listing (pairwise disjoint; union == all 16; "
          "every record EXACTLY ONCE; no None case — the int is "
    ...
    for seed in dist:
        _, body = call_json("GET", byseed(seed))
        expected = [e for e in evals if e["seed"] == seed]
    ...
    print("== LIVE I: (G) M4/M24/M28/M30/M36/M38/M47 regressions ==")
    ...
    t47_by = {v: call_json("GET", f"{BASE}/models/{MODEL}/"
              ...
              for v in M47_DIST}
    check("I1/G1 M4/M24/M28/M30/M36/M38/M47 evaluation surfaces "
          "unchanged (listing identical to LIVE A, 3/3/3 "
          "by-checkpoint, 16 by-dataset, 16 by-tokenizer, by-split "
          "14/2/0, by-state-kind checkpoint->9 / current->7, "
          "by-truncated false->15 / true->1, all with listing parity)",
    ...
    check("M1 OpenAPI exactly 80 paths, the new path exactly once "
          "(after by-truncated, before the generic evaluation-detail "
          "route; integer parameter schema; EvaluationRecord items; "
          "the M47/M46/M45/M44/M43/M42/M41 routes still present "
          "exactly once)",
          code_sp2 == 200 and len(spec2["paths"]) == 80
          ...
          and list(spec2["paths"]).index(M47_PATH)
    ...
    _, grouped2 = call_json("GET", byseed(11))
    ...
    print()
    if FAILURES:
        print(f"M48 live smoke FAILED: {len(FAILURES)} check(s) "
              "failed:")
        for f in FAILURES:
            print("  -", f)
        return 1
    print("M48 live smoke OK: narrow read-only MODEL-SCOPED by-seed "
          "grouping of the M4 evaluation history over the persisted "
          "REQUIRED integer (distribution DISCOVERED live: 11->4 / "
          "1->3 / 7->2 / 3->2 / 7002->2 / 7101->2 / 12->1 among 16 "
          "evaluations, SEVEN populated groups — exact listing parity, "
          "persisted seed VERBATIM, never recalculated, never "
          "normalized, never derived from the config dict or any "
          "other field, no quality judgment, authoritative "
          "(created_at, eval_id) order preserved, every record "
          "exactly once, TRUE disjoint partition with no None case — "
          "the OPEN integer value axis with no registry), natural "
          "valid-empty 200 + [] for an unmatched integer on both "
          "models, schema-level 422 for non-integer spellings "
          "(pre-handler, even for an unknown model), 404 for unknown "
          "model with a valid integer, deterministic byte-identical "
          "x3 repeats, M3-M47 surfaces + dashboard hash + registries "
          "+ OpenAPI 80 unchanged, ZERO production storage growth) "
