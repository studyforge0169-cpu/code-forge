"""M42 live HTTP smoke against the production FORGE_ROOT
(/home/user/ai-model-forge-data). M42 is READ-ONLY: it adds one narrow
MODEL-SCOPED access path,
GET /models/{id}/workflows/by-status/{status}, answering "which
immutable workflow runs of this model ended with this terminal
status?" — the model's authoritative M11 listing filtered by the
persisted top-level status recorded in each WorkflowRecord (the
schema enum completed/failed/stopped, the terminal state persisted at
run end by the M7 orchestration; NEVER inferred from stage results,
failed stage ids, timestamps, artifact existence or recipe
information; nothing is re-executed; membership never inferred from
filenames or hashes), registered BEFORE the generic
/workflows/{workflow_id} route and alongside by-recipe (M35).
Unsupported status values are rejected with 422 by the schema enum
BEFORE the handler runs (never a registry-style 404; true even for an
unknown model); an unknown model with a VALID status is 404; a valid
status with zero matching runs is 200 + [] (never 404). Each record
appears EXACTLY ONCE and the groups partition the full M11 listing.
The smoke must DISCOVER the authoritative status distribution from
the live M11 listing and the schema enum (not assume it from an old
report), prove exact listing parity for ALL THREE groups, the natural
valid-empty cases, clean 404/422 separation, byte-identical repeats
(x3), unchanged M2-M41 surfaces + dashboard hash + registries +
OpenAPI 74, and ZERO production storage growth.

Production facts (re-derived live at LIVE A/B; the authoritative
sources are the M11 listing and the WorkflowStatus schema enum):
model 4a0a871886ef owns 13 workflow runs in ASCENDING (created_at,
workflow_id) order with persisted status distribution completed -> 9,
failed -> 3, stopped -> 1 (all three groups non-empty); model
b5bc905326b6 has NO workflow runs; M17 dashboard result_hash
...
SHA256 inventory saved to /tmp/m42-smoke-baseline-inventory.json +
model/checkpoint registries + M4/M5/M6/M11/M15/M16/M18-M41 pre-state
+ dashboard + OpenAPI 74 pre-state + the known pair returns 200.

LIVE B (authoritative distribution discovery): from the live M11
listing and the OpenAPI WorkflowStatus schema enum, discover the
per-status record distribution and the enum values; print them;
cross-check against the audited facts.

LIVE C (A — known statuses): completed, failed AND stopped -> 200
with the EXACT discovered records (9/3/1); parity with the M11
listing filtered locally; verbatim detail-getter payloads;
(created_at, workflow_id) ASC.

LIVE D (B — repeatability): three GETs raw-byte-identical.

LIVE E (C — valid empty statuses): ALL enum statuses -> 200 + []
under b5bc905326b6 (it has no workflow runs at all).

LIVE F (D/E — unknown ids / unsupported values): unknown model +
valid status -> 404; unsupported status value -> 422 on the known
model (incl. case variant + numeric + the deliberately-unmodelled
'running'); unsupported status + UNKNOWN model -> 422 (schema
validation fires pre-handler).

LIVE G (F — partition): the groups over all enum statuses partition
the full M11 listing exactly (9 + 3 + 1 == 13; every record EXACTLY
ONCE; no record in two groups; every record's persisted status
verbatim).

LIVE H (G — ordering): every group's order equals the authoritative
M11 listing order restricted to that status.

LIVE I (H — M35/M11 regressions): workflows 2 by-recipe
(m12-live-suite) + the M11 listing unchanged, all with listing
parity.

LIVE J (I — M22-M41 regressions): evaluations 3/3/3 by-checkpoint +
...
gate decisions 11 total with by-decision passed->7 / failed->4 + 4
...
LIVE M (M — OpenAPI + storage zero drift): OpenAPI exactly 74 paths,
new path once (after the by-recipe workflow route, before the generic
workflow route; the M41 gate-decisions-by-decision route still
present exactly once); every pre-existing file byte-identical, ZERO
new files, zero .tmp, totals unchanged 96/4,002,745/0.

Exit code 0 = all checks passed. Start the server first:
    FORGE_ROOT=/home/user/ai-model-forge-data python -m uvicorn \\
        app.api:app --host 127.0.0.1 --port 8763 --log-level warning
"""
...
BASE = "http://127.0.0.1:8763/api/v1"
SITE = "http://127.0.0.1:8763"
MODEL = "4a0a871886ef"
OTHER_MODEL = "b5bc905326b6"          # exists, NO workflow runs
...
NEW_PATH = "/api/v1/models/{model_id}/workflows/by-status/{status}"
GENERIC_PATH = "/api/v1/models/{model_id}/workflows/{workflow_id}"
M35_PATH = "/api/v1/models/{model_id}/workflows/by-recipe/{recipe_id}"
M41_PATH = ("/api/v1/models/{model_id}/gates/decisions/by-decision/"
            "{decision}")
ROOT = Path("/home/user/ai-model-forge-data")
INVENTORY = Path("/tmp/m42-smoke-baseline-inventory.json")
...
def bystatus(status: str, model: str = MODEL) -> str:
    return f"{BASE}/models/{model}/workflows/by-status/{status}"
...
def main() -> int:
    ...
    code_wf, workflows = call_json("GET", f"{BASE}/models/{MODEL}/"
                                          "workflows")
    code_wfo, workflows_o = call_json("GET", f"{BASE}/models/"
                                            f"{OTHER_MODEL}/workflows")
    ...
    check("A4 M4/M5/M6/M11/M15/M16/M18-M41 pre-state intact (13 "
          "workflow runs, other model 0, 11 gate decisions, 16 "
          "evaluations, 8 comparisons, 4 samples, 7 recipes, 2 global "
          "m12-live-suite runs, 2 by-recipe runs, 4 by-comparison "
          "decisions, 1 by-policy decision, 7 by-decision passed, 2 "
          ...
          "3 improved-verdict comparisons, 2 greedy-strategy samples)",
          code_wf == 200 and len(workflows) == 13 and code_wfo == 200
          and workflows_o == [] and code_gd == 200
          and len(gdecisions) == 11 and code_ev == 200
    ...
    check("A5 M17 dashboard hash equals the full known value",
          ...
          d0["result_hash"][:16])
    check("A6 OpenAPI pre-state: 74 paths, the new by-status path "
          "registered exactly once with only GET",
          code_sp == 200 and len(paths) == 74
          and list(paths).count(NEW_PATH) == 1 and set(new_ops) == {"get"}
          and new_ops["get"]["tags"] == ["workflows"])
    code_a, raw_a = call("GET", bystatus("completed"))
    check("A7 the known pair (4a0a871886ef + completed) -> 200",
          code_a == 200, f"{code_a}")

    print("== LIVE B: authoritative distribution discovery ==")
    schema = spec["components"]["schemas"]["WorkflowStatus"]
    enum = tuple(schema.get("enum", ()))
    by_st: dict[str, list[str]] = {}
    for w in workflows:
        by_st.setdefault(w["status"], []).append(w["workflow_id"])
    dist_str = ", ".join(f"{k} -> {len(v)}"
                         for k, v in sorted(by_st.items()))
    print(f"    discovered: {len(workflows)} workflow runs of {MODEL} "
          f"per status: {dist_str}")
    print(f"    discovered: WorkflowStatus enum values: {list(enum)}")
    check("B1 M11 listing pre-state: 13 workflow runs in ASCENDING "
          "(created_at, workflow_id) order; distribution completed->9, "
          "failed->3, stopped->1; other model has none",
          code_wf == 200 and len(workflows) == 13
          and [(w["created_at"], w["workflow_id"]) for w in workflows]
          == sorted((w["created_at"], w["workflow_id"])
                    for w in workflows)
          and {k: len(by_st.get(k, [])) for k in enum}
          == dict(M42_DIST)
          and all(w["status"] in enum for w in workflows)
          and code_wfo == 200 and workflows_o == [])
    check("B2 the schema enum holds exactly completed/failed/stopped",
          set(enum) == set(EXPECTED_STATUSES) and len(enum) == 3,
          str(enum))
    ...
    for st in EXPECTED_STATUSES:
        code_g, raw_g = call("GET", bystatus(st))
        groups[st] = json.loads(raw_g)
        ok_groups = ok_groups and code_g == 200 and groups[st] == \
            [w for w in workflows if w["status"] == st]
    check("C1/A1 ALL THREE status responses == M11 listing filtered "
          "locally by the persisted status (no missing / extra / "
          "duplicate; every record EXACTLY ONCE; the persisted status "
          "travels VERBATIM — never inferred from stage results or "
          "artifacts)",
          ok_groups
          and all(len(groups[st]) == M42_DIST[st]
                  and all(w["model_id"] == MODEL
                          and w["status"] == st
                          for w in groups[st])
                  for st in EXPECTED_STATUSES),
          "/".join(str(len(groups[st])) for st in EXPECTED_STATUSES))
    ok_detail = True
    for st in EXPECTED_STATUSES:
        for w in groups[st]:
            code_d, one = call_json("GET", f"{BASE}/models/{MODEL}/"
                                           f"workflows/"
                                           f"{w['workflow_id']}")
            if code_d != 200 or one != w:
                ok_detail = False
                check("C2/A1 verbatim payload parity with the M11 "
                      "detail getter", False, w["workflow_id"])
                break
        if not ok_detail:
            break
    if ok_detail:
        check("C2/A1 verbatim payload parity with the M11 detail getter "
              "for all 13 records (stages, transitions, terminal_reason, "
              "result_hash and recipe provenance included)", True)

    print("== LIVE D: (B) repeatability ==")
    _, r1 = call("GET", bystatus("completed"))
    _, r2 = call("GET", bystatus("completed"))
    _, r3 = call("GET", bystatus("completed"))
    check("D1/B1 three GETs raw-byte-identical", r1 == r2 == r3 == raw_a)

    print("== LIVE E: (C) valid empty statuses ==")
    ok_other = True
    for st in enum:
        c_o, body_o = call_json("GET", bystatus(st, OTHER_MODEL))
        ok_other = ok_other and c_o == 200 and body_o == []
    check("E1/C1 ALL enum statuses under b5bc905326b6 -> 200 + [] "
          "(its M11 listing is empty; the status value is "
          "schema-validated even when the model has no history)",
          ok_other and workflows_o == [])

    print("== LIVE F: (D/E) unknown ids / unsupported values ==")
    c1, _ = call_json("GET", bystatus("completed", "no-such-model-42"))
    check("F1/D1 unknown model + VALID status -> 404",
          c1 == 404, f"{c1}")
    c2, b2 = call_json("GET", bystatus("COMPLETED"))
    c3, b3 = call_json("GET", bystatus("compl%20eted"))
    c4, b4 = call_json("GET", bystatus("1"))
    check("F2/E1 unsupported status values -> 422 on the known model "
          ...
          f"{c2}/{c3}/{c4}")
    c5, _ = call_json("GET", bystatus("running", "no-such-model-42"))
    check("F3/E1 unsupported status (the deliberately-unmodelled "
          "'running') + UNKNOWN model -> 422 (validation fires "
          "pre-handler; the 422/404 precedence is the schema's, not a "
          "manual conversion)",
          c5 == 422, f"{c5}")
    ...
    ok_groups2 = True
    for st in enum:
        _, body = call_json("GET", bystatus(st))
        ok_groups2 = ok_groups2 and all(w["status"] == st
                                        for w in body)
        seen |= {w["workflow_id"] for w in body}
    all_ids = {w["workflow_id"] for w in workflows}
    check("G1/F1 the groups over all enum statuses partition the FULL "
          "M11 listing exactly (9 + 3 + 1 == 13; every record EXACTLY "
          "ONCE; no record in two groups)",
          ok_groups2 and seen == all_ids and len(seen) == 13)

    print("== LIVE H: (G) ordering ==")
    ok_order = True
    for st in enum:
        _, body = call_json("GET", bystatus(st))
        expected = [w for w in workflows if w["status"] == st]
    ...
    check("H1/G1 every group preserves the authoritative M11 "
          "(created_at, workflow_id) ASCENDING order exactly", ok_order)

    print("== LIVE I: (H) M35/M11 regressions ==")
    ...
    _, workflows2 = call_json("GET", f"{BASE}/models/{MODEL}/"
                                     "workflows")
    check("I1/H1 M35 by-recipe + the M11 listing unchanged (2 "
          "by-recipe with listing parity, listing identical to LIVE A)",
          byrec2 == byrec_runs and len(byrec2) == 2
          and byrec2 == [w for w in workflows
                         if w["recipe_id"] == KNOWN_RECIPE]
          and workflows2 == workflows)

    print("== LIVE J: (I) M22-M41 regressions ==")
    ...
    _, gd232 = call_json("GET", f"{BASE}/models/{MODEL}/gates/"
                                f"decisions/by-policy/{POLICY}")
    check("J6/I1 M41/M34/M23 gate-decision histories unchanged "
          "(by-decision passed->7 / failed->4 with listing parity, 4 "
          "by-comparison, 1 by-policy)",
          gd41p2 == gd41p and len(gd41p2) == M41_DIST["passed"]
    ...
    check("M1 OpenAPI exactly 74 paths, the new path exactly once "
          "(after the by-recipe workflow route, before the generic "
          "workflow route; the M41 gate-decisions-by-decision route "
          "still present exactly once)",
          code_sp2 == 200 and len(spec2["paths"]) == 74
          and list(spec2["paths"]).count(NEW_PATH) == 1
          and set(spec2["paths"][NEW_PATH]) == {"get"}
          and list(spec2["paths"]).index(M35_PATH)
          < list(spec2["paths"]).index(NEW_PATH)
          < list(spec2["paths"]).index(GENERIC_PATH)
          and list(spec2["paths"]).count(M41_PATH) == 1)
    _, grouped2 = call_json("GET", bystatus("completed"))
    check("M2 by-status still deterministic at the end",
          grouped2 == groups["completed"])
    ...
    print()
    if FAILURES:
        print(f"M42 live smoke FAILED: {len(FAILURES)} check(s) "
              "failed:")
        for f in FAILURES:
            print("  -", f)
        return 1
    print("M42 live smoke OK: narrow read-only MODEL-SCOPED "
          "by-status grouping of the M11 workflow history "
          "(distribution DISCOVERED live: completed->9, failed->3, "
          "stopped->1 among 13 workflow runs — exact listing parity, "
          "persisted status identity VERBATIM (never inferred from "
          "stage results or artifacts, never re-executed), "
          "authoritative order, every record exactly once, full "
          "partition —, schema-enum 422 for unsupported status values "
          "even pre-handler and even for an unknown model, valid "
          "empty 200 + [] for every status under b5bc905326b6, clean "
          "404 for unknown model with a valid status, deterministic "
          "byte-identical x3 repeats, M12/M16/M22-M41 surfaces + "
          "dashboard hash + registries + OpenAPI 74 unchanged, ZERO "
