...
def test_recipe_api_lifecycle_immutability_and_reuse(api_client):
    ...
    # immutability: no update endpoints exist (405); DELETE is the
    # M71 verified retention (scope: unknown id -> 404, nothing
    # deleted — the recipe below is still referenced by its runs)
    assert api_client.put(f"{RECIPES}/api12-life").status_code == 405
    assert api_client.delete(f"{RECIPES}/no-such").status_code == 404
    ...
    # M71: the recipe is REFERENCED by its two runs -> blocked 409
    # with the ordered typed blocker list (no force, no cascade)
    blocked = api_client.delete(f"{RECIPES}/api12-life")
    assert blocked.status_code == 409
    detail = blocked.json()["detail"]
    assert detail["family"] == "workflow_recipe"
    assert detail["definition_id"] == "api12-life"
    assert sorted((b["category"], b["reference_id"]) for b in
                  detail["blockers"]) == sorted(
        ("workflow", w["workflow_id"]) for w in (w1, w2))
    assert api_client.get(f"{RECIPES}/api12-life").status_code == 200
