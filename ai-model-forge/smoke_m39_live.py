"""M39 live HTTP smoke against the production FORGE_ROOT
(/home/user/ai-model-forge-data). M39 is READ-ONLY: it adds one narrow
MODEL-SCOPED access path, GET /models/{id}/comparisons/by-verdict/{verdict},
answering "which immutable M5 comparison records of this model produced
this verdict?" — the model's authoritative M5 listing filtered by the
persisted top-level verdict recorded in each ComparisonRecord (the
schema enum improved/regressed/unchanged — the immutable loss-only
judgment persisted at run time; NEVER recalculated from loss deltas,
per-side losses, tolerances, checkpoint ids or hashes, never executed),
...
verdict values are rejected with 422 by the schema enum BEFORE the
handler runs (never a registry-style 404; true even for an unknown
model); an unknown model with a VALID verdict is 404; a valid verdict
with zero matching records is 200 + [] (never 404). Each record
appears EXACTLY ONCE and the groups partition the full M5 listing.
The smoke must DISCOVER the authoritative verdict distribution from
the live M5 listing and the schema enum (not assume it from an old
...
(x3), unchanged M2-M38 surfaces + dashboard hash + registries +
OpenAPI 71, and ZERO production storage growth.

Production facts (re-derived live at LIVE A/B; the authoritative
sources are the M5 listing and the ComparisonVerdict schema enum):
model 4a0a871886ef owns 8 comparisons in ASCENDING (created_at,
comparison_id) order with persisted verdict distribution
improved -> 3, unchanged -> 3, regressed -> 2 (ALL THREE groups
non-empty); model b5bc905326b6 has NO comparisons; M17 dashboard
...
SHA256 inventory saved to /tmp/m39-smoke-baseline-inventory.json +
model/checkpoint registries + M4/M5/M6/M11/M16/M18-M38 pre-state +
dashboard + OpenAPI 71 pre-state + the known pair returns 200.
...
per-verdict record distribution and the enum values; print them;
cross-check against the audited facts.

LIVE C (A — known verdicts): improved AND unchanged AND regressed ->
200 with the EXACT discovered records (3/3/2); parity with the M5
listing filtered locally; verbatim detail-getter payloads;
(created_at, comparison_id) ASC.

LIVE D (B — repeatability): three GETs raw-byte-identical.

LIVE E (C — valid empty verdicts): ALL enum verdicts -> 200 + [] under
b5bc905326b6 (it has no comparisons at all).

LIVE F (D/E — unknown ids / unsupported values): unknown model + valid
verdict -> 404; unsupported verdict value -> 422 on the known model
(incl. case variant + numeric); unsupported verdict + UNKNOWN model ->
422 (schema validation fires pre-handler).

LIVE G (F — partition): the groups over all enum verdicts partition
the full M5 listing exactly (3 + 3 + 2 == 8; every record EXACTLY
ONCE; no record in two groups; every record's persisted verdict
verbatim).

LIVE H (G — ordering): every group's order equals the authoritative
M5 listing order restricted to that verdict.

LIVE I (H — M26/M29/M31/M37 regressions): comparisons 6/5/1
by-checkpoint + 8 by-dataset + 8 by-tokenizer + by-split
validation->8 / train->0 / test->0, all with listing parity.

LIVE J (I — M22-M34 + M38 regressions): evaluations 3/3/3
by-checkpoint + 16 by-dataset + 16 by-tokenizer + by-split
validation->14 / train->2 / test->0; M38 by-state-kind
checkpoint->9 / current->7 + parity; suite runs 10 by-suite + 10
by-checkpoint + summary; samples 4/0/0 by-checkpoint + 4
...
LIVE M (M — OpenAPI + storage zero drift): OpenAPI exactly 71 paths,
new path once (after the by-split comparison route, before the generic
comparison route; the M38 evaluations-by-state-kind route still
...
Exit code 0 = all checks passed. Start the server first:
    FORGE_ROOT=/home/user/ai-model-forge-data python -m uvicorn \\
        app.api:app --host 127.0.0.1 --port 8760 --log-level warning
"""
...
BASE = "http://127.0.0.1:8760/api/v1"
SITE = "http://127.0.0.1:8760"
MODEL = "4a0a871886ef"
...
EXPECTED_DIST = {"improved": 3, "unchanged": 3, "regressed": 2}
NEW_PATH = ("/api/v1/models/{model_id}/comparisons/by-verdict/"
            "{verdict}")
GENERIC_PATH = "/api/v1/models/{model_id}/comparisons/{comparison_id}"
SPLIT_PATH = "/api/v1/models/{model_id}/comparisons/by-split/{split}"
M38_PATH = ("/api/v1/models/{model_id}/evaluations/by-state-kind/"
            "{state_kind}")
ROOT = Path("/home/user/ai-model-forge-data")
INVENTORY = Path("/tmp/m39-smoke-baseline-inventory.json")
...
def byverdict(verdict: str, model: str = MODEL) -> str:
    return f"{BASE}/models/{model}/comparisons/by-verdict/{verdict}"
...
def main() -> int:
    ...
    check("A4 M4/M5/M6/M11/M16/M18-M38 pre-state intact (16 "
          "evaluations, 8 comparisons, other model 0 for both, 7 "
    ...
    check("A5 M17 dashboard hash equals the full known value",
          ...
          d0["result_hash"][:16])
    check("A6 OpenAPI pre-state: 71 paths, the new by-verdict path "
          "registered exactly once with only GET",
          code_sp == 200 and len(paths) == 71
    ...
    for c in comps:
        by_verdict.setdefault(c["verdict"], []).append(
            c["comparison_id"])
    dist_str = ", ".join(f"{k} -> {len(v)}"
                         for k, v in sorted(by_verdict.items()))
    print(f"    discovered: {len(comps)} comparisons of {MODEL} per "
          f"verdict: {dist_str}")
    print(f"    discovered: ComparisonVerdict enum values: "
          f"{list(enum)}")
    check("B1 M5 listing pre-state: 8 comparisons in ASCENDING "
          "(created_at, comparison_id) order; distribution "
          "improved->3, unchanged->3, regressed->2; other model has "
          "none",
          code_cp == 200 and len(comps) == 8
          and [(c["created_at"], c["comparison_id"]) for c in comps]
          == sorted((c["created_at"], c["comparison_id"])
                    for c in comps)
          and {v: len(by_verdict.get(v, [])) for v in enum}
          == dict(EXPECTED_DIST)
          and all(c["verdict"] in enum for c in comps)
          and code_cpo == 200 and comps_o == [])
    check("B2 the schema enum holds exactly improved/regressed/"
          "unchanged",
    ...
    for v in EXPECTED_VERDICTS:
        ...
        ok_groups = ok_groups and code_g == 200 and groups[v] == \
            [c for c in comps if c["verdict"] == v]
    check("C1/A1 ALL THREE verdict responses == M5 listing filtered "
          "locally by the persisted verdict (no missing / extra / "
          "duplicate; every record EXACTLY ONCE; the persisted verdict "
          "travels VERBATIM — never recalculated)",
          ok_groups
          and all(len(groups[v]) == EXPECTED_DIST[v]
                  and all(c["model_id"] == MODEL and c["verdict"] == v
                          for c in groups[v])
                  for v in EXPECTED_VERDICTS),
        ...
        for c in groups[v]:
            ...
            if code_d != 200 or one != c:
                ...
                check("C2/A1 verbatim payload parity with the M5 "
                      "detail getter", False, c["comparison_id"])
                break
        if not ok_detail:
            break
    if ok_detail:
        check("C2/A1 verbatim payload parity with the M5 detail getter "
              "for all 8 records (verdict/per-side losses included)",
              True)
    ...
    check("E1/C1 ALL enum verdicts under b5bc905326b6 -> 200 + [] "
          "(its M5 listing is empty; the verdict value is "
          "schema-validated even when the model has no history)",
          ok_other and comps_o == [])

    print("== LIVE F: (D/E) unknown ids / unsupported values ==")
    c1, _ = call_json("GET", byverdict("improved", "no-such-model-39"))
    check("F1/D1 unknown model + VALID verdict -> 404",
          c1 == 404, f"{c1}")
    c2, b2 = call_json("GET", byverdict("im%20proved"))
    c3, b3 = call_json("GET", byverdict("IMPROVED"))
    ...
    check("F2/E1 unsupported verdict values -> 422 on the known model "
          ...
          f"{c2}/{c3}/{c4}")
    c5, _ = call_json("GET", byverdict("better", "no-such-model-39"))
    check("F3/E1 unsupported verdict + UNKNOWN model -> 422 "
    ...
    for v in enum:
        ...
        ok_groups2 = ok_groups2 and all(c["verdict"] == v
                                        for c in body)
        seen |= {c["comparison_id"] for c in body}
    all_ids = {c["comparison_id"] for c in comps}
    check("G1/F1 the groups over all enum verdicts partition the FULL "
          "M5 listing exactly (3 + 3 + 2 == 8; every record EXACTLY "
          "ONCE; no record in two groups)",
          ok_groups2 and seen == all_ids and len(seen) == 8)

    print("== LIVE H: (G) ordering ==")
    ok_order = True
    for v in enum:
        _, body = call_json("GET", byverdict(v))
        expected = [c for c in comps if c["verdict"] == v]
    ...
    print("== LIVE I: (H) M26/M29/M31/M37 regressions ==")
    ok_ck = True
    ...
    cps_by = {sp: call_json("GET", f"{BASE}/models/{MODEL}/"
              ...
              for sp in M36_DIST}
    check("I1/H1 M26/M29/M31/M37 comparison histories unchanged (025e"
          "->6 / 30a8->5 / 0511->1 by-checkpoint, 8 by-dataset, 8 "
    ...
    print("== LIVE J: (I) M22-M34 + M38 regressions ==")
    ...
    check("M1 OpenAPI exactly 71 paths, the new path exactly once "
          "(after the by-split comparison route, before the generic "
          "comparison route; the M38 evaluations-by-state-kind route "
          "still present exactly once)",
          code_sp2 == 200 and len(spec2["paths"]) == 71
          ...
          and list(spec2["paths"]).count(M38_PATH) == 1)
    ...
    print()
    if FAILURES:
        print(f"M39 live smoke FAILED: {len(FAILURES)} check(s) "
              "failed:")
        for f in FAILURES:
            print("  -", f)
        return 1
    print("M39 live smoke OK: narrow read-only MODEL-SCOPED "
          "by-verdict grouping of the M5 comparison history "
          "(distribution DISCOVERED live: improved->3, unchanged->3, "
          "regressed->2 among 8 comparisons — exact listing parity, "
          "persisted verdict identity VERBATIM (never recalculated "
          "from losses), authoritative order, every record exactly "
          "once, full three-group partition —, schema-enum 422 for "
          "unsupported verdict values even pre-handler and even for "
          "an unknown model, valid empty 200 + [] for every verdict "
          "under b5bc905326b6, clean 404 for unknown model with a "
          "valid verdict, deterministic byte-identical x3 repeats, "
          "M12/M16/M22-M38 surfaces + dashboard hash + registries + "
          "OpenAPI 71 unchanged, ZERO production storage growth) "
