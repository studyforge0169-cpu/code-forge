...
# global listings and the EIGHT model-scoped record listings.
# Hyphenated or shortened aliases (suite-runs, quality, ...) are
# NOT exposed. A drift test pins this table to the engine's actual
# list_* facade set by signature shape (no missing surface, no
# extra, no wrong scope); the CLI never constructs a listing itself.
_LIST_DISPATCH = {
